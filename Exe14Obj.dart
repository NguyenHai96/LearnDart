...
class Student {
  ...
  void import() {
    ...
    do {
      try {
        ...
        if (!(this.birthday.length == 10 &&
            int.parse(this.birthday[0]).runtimeType == int &&
            int.parse(this.birthday[1]).runtimeType == int &&
            this.birthday[2] == '/' &&
            int.parse(this.birthday[3]).runtimeType == int &&
            int.parse(this.birthday[4]).runtimeType == int &&
            this.birthday[5] == '/' &&
            int.parse(this.birthday[6]).runtimeType == int &&
            int.parse(this.birthday[7]).runtimeType == int &&
            int.parse(this.birthday[8]).runtimeType == int &&
            int.parse(this.birthday[9]).runtimeType == int)) {
          throw new InvalidDOBException(mess: 'Ngay sinh nhap khong hop le!');
        }
      } catch (err) {
        print('Lỗi nhập ngày tháng năm sinh $err');
      }
    } while (this.birthday.length != 10 ||
        int.parse(this.birthday[0]).runtimeType != int ||
        int.parse(this.birthday[1]).runtimeType != int ||
        this.birthday[2] != '/' ||
        int.parse(this.birthday[3]).runtimeType != int ||
    ...
    } while (!(int.parse(this.phone) == int ||
        this.phone.length == 10 ||
        this.phone.substring(0, 2) == '090' ||
        this.phone.substring(0, 2) == '098' ||
        this.phone.substring(0, 2) == '091' ||
...
class Selective {
  List<GoodStudent> dataGood = [];
  List<NormalStudent> dataNormal = [];
  ...
  void collectStudent() {
    ...
    } while (quantity > 15 || quantity < 11);
    if (quantity == dataGood.length) {
      dataGood.forEach((element) {
        element.ShowMyInfor();
      });
    } else {
      if (dataGood.length < quantity) {
        dataGood.forEach((element) {
          element.ShowMyInfor();
        });
        int missingQuantity = quantity - dataGood.length;
        this.dataNormal.sort((a, b) => b.getScore().compareTo(a.getScore()));
        for (int i = 0; i < missingQuantity; i++) {
          dataNormal[i].ShowMyInfor();
        }
      } else {
        this.dataGood.sort((a, b) => b.getScore().compareTo(a.getScore()));
        for (int i = 0; i < quantity; i++) {
          dataGood[i].ShowMyInfor();
        }
      }
    }
  }
